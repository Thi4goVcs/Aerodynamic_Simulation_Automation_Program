...
REM Standard/), so it can find the OpenFOAM case templates at runtime.
...
python -m PyInstaller --onefile --windowed --name AeroSimApp --collect-all customtkinter --distpath . --noconfirm Run.py
if errorlevel 1 (
...
echo Keep it here, next to Standard/, Results/, etc. -- it looks for them
